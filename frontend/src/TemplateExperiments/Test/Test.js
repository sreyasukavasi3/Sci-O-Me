...
function Test(props) {
	...
	useMemo(() => {
		...
		.then(response => {
			...
			const presentList = experiment.present.map(imagePath => "'" + imagePath + "'");
			const respondList = experiment.respond.map(imagePath => "'" + imagePath + "'");
			const corrAnsList = experiment.corrAns;
